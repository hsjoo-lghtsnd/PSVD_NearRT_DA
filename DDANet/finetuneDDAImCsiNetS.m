...
%   .ValFrequency
    ...
    config = struct();
    config.nTx = nTx;
    config.nSubbands = size(XFieldTrain,1) / (2*nTx);
    assert(abs(config.nSubbands - round(config.nSubbands)) < 1e-12, ...
        'Input dimension is not compatible with 2*nTx*nSubbands.');
    config.nSubbands = round(config.nSubbands);
        ...
        while hasdata(mbq)
            ...
            if mod(iteration, opts.ValFrequency) == 0 && ~isempty(XFieldVal)
                [valLoss, valCos] = evaluateImCsiNetM( ...
                    encoderNet, decoderNet, XFieldVal, config, ...
